...
# KnowledgeExtractor 类和规则定义保持不变

def build_knowledge_graph(extracted_knowledge, article_id):
    G = nx.Graph()

    article_node_id = f"文章_{article_id}"
    G.add_node(article_node_id, name=f"文章_{article_id}", type="文章")

    for key, values in extracted_knowledge.items():
        if isinstance(values, list):
            for value in values:
                node_id = f"{key}_{value}_{uuid.uuid4().hex[:8]}"
                G.add_node(node_id, name=value, type=key)
                G.add_edge(article_node_id, node_id, relation=key)
        else:
            node_id = f"{key}_{values}_{uuid.uuid4().hex[:8]}"
            G.add_node(node_id, name=values, type=key)
            G.add_edge(article_node_id, node_id, relation=key)

    return G

def save_to_neo4j(G, uri, username, password):
    ...
    for node in G.nodes():
        ...
        # 检查节点是否已存在
        existing_node = matcher.match(node_type, name=node_name).first()
        if existing_node is None:
            neo4j_node = Node(node_type, id=node, name=node_name)
            tx.create(neo4j_node)
            nodes_created += 1
        else:
            neo4j_node = existing_node

        # 将节点对象存储在图中，以便后续创建关系时使用
        G.nodes[node]['neo4j_obj'] = neo4j_node

    for edge in G.edges():
        start_node = G.nodes[edge[0]].get('neo4j_obj')
        end_node = G.nodes[edge[1]].get('neo4j_obj')

        if start_node and end_node:
            relation_type = G.edges[edge].get('relation', 'RELATED_TO')
            relation = Relationship(start_node, relation_type, end_node)
            tx.merge(relation)
            relationships_created += 1

    tx.commit()
    logging.info(f"Data saved to Neo4j successfully. Created {nodes_created} nodes and {relationships_created} relationships.")

# 主程序
if __name__ == "__main__":
    # 读取CSV文件
    file_path = r"H:\智融通检，语界无疆\toutiao_recommendations.csv"
    ...
    for index, row in df.iterrows():
        ...
        if not full_text.strip():
            logging.warning(f"Empty text for article {index + 1}, skipping...")
            continue

        extracted_knowledge = extractor.extract(full_text)

        # 构建单篇文章的知识图谱
        G_article = build_knowledge_graph(extracted_knowledge, index)

        # 将单篇文章的图谱合并到总图谱中
        G_total = nx.compose(G_total, G_article)
    ...
    # 设置输出文件夹
    output_folder = r"H:\智融通检，语界无疆\1月7号输出"
    if not os.path.exists(output_folder):
        os.makedirs(output_folder)

    # 保存最终的知识图谱
    nx.write_gexf(G_total, os.path.join(output_folder, "final_knowledge_graph.gexf"))

    # 保存到Neo4j
    neo4j_uri = "bolt://localhost:7687"
    neo4j_username = "neo4j"
    neo4j_password = "Zrf,123456"
    save_to_neo4j(G_total, neo4j_uri, neo4j_username, neo4j_password)

    # 统计和输出图谱信息
    relation_counts = {}
    for _, _, data in G_total.edges(data=True):
    ...
    for node in G_total.nodes():
        entity_counts[G_total.nodes[node].get('name', node)] = G_total.degree(node)
    for entity, count in sorted(entity_counts.items(), key=lambda x: x[1], reverse=True)[:10]:
        logging.info(f"{entity}: {count}")

    with open(os.path.join(output_folder, "final_graph_statistics.txt"), 'w', encoding='utf-8') as f:
        f.write(f"Total nodes: {G_total.number_of_nodes()}\n")
        f.write(f"Total edges: {G_total.number_of_edges()}\n")
        f.write(f"Unique entities: {len(set(G_total.nodes()))}\n")
        f.write(f"Unique relations: {len(set(e[2]['relation'] for e in G_total.edges(data=True)))}\n")
    ...
    logging.info(f"\nFinal graph statistics saved to {os.path.join(output_folder, 'final_graph_statistics.txt')}")